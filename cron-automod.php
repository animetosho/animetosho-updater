...
// since we don't have an index on dateline, use idx_dateline to reduce the scope of the search
// for whatever reason, MySQL can't optimise properly if this is a subquery, so execute it separately
$minId = $db->selectGetField('toto', 'MIN(id)', 'idx_dateline < -'.($time - 86400*2.5));

// only check at most once every 3 hours, up to 1.5 days (up to 12 checks)
$totos = $db->selectGetAll('toto', 'id', 'deleted!=-1 AND dateline > '.($time - 86400*1.5).' AND id >= '.$minId.' AND lastchecked < '.($time - 3600*3), 'id,tosho_id,nyaa_id,nyaa_subdom,anidex_id,nekobt_id,sigfid,deleted,btih,ulcomplete,stored_nzb', array('order' => 'lastchecked ASC', 'limit' => 50));
	...
	if($r === null) { // deleted
		...
		if(!$toto['deleted'] && ($dupe = find_dupe($id, $toto['btih']))) {
			...
			if($toto['ulcomplete'] >= 0 && $dupe['ulcomplete'] < 0) {
				...
				info('Deleted duplicate itemid='.$id.' (ulq:'.$toto['ulcomplete'].') setting non-dupe id='.$dupe['id'].' (ulq:'.$dupe['ulcomplete'].')', 'automod-dupe');
			}
		} elseif(!$toto['deleted']) {
			info('Deleting itemid='.$id, 'automod');
		}
	} elseif($r) {
		if($toto['deleted']) {
			$update['deleted'] = 0;
