...
class TransmissionRPC
{
  ...
  {
    if ( !is_array( $ids ) && $ids !== 'recently-active' ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
  ...
  protected function request( $method, $arguments, $in_recur=false )
  {
    ...
    for ( $tries = 0; $tries < 5; ++$tries ) {
      if ( $fp = @fopen( $this->url, 'r', false, $context ) ) break;	// Open a filepointer to the data, and use fgets to get the result
      sleep( 10 * ($tries+1) );
    }
    if ( $fp ) {
      $response = '';
    ...
    // fix Transmission bug of not encoding some chars properly
    $response = preg_replace_callback('~[\x00-\x07\x0b\x0e-\x1f]~', function($c) {
        return '\\u00'.bin2hex($c[0]);
    }, $response);
