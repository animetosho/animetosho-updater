...
function numberdecode($wholefile, $start)
{
	...
	while (true)
	{
		if(!isset($wholefile[$offset]) || !is_int($offset)) return array(false);
		if ($wholefile[$offset] >= '0' && $wholefile[$offset] <= '9')
		{
			$ret[0] .= $wholefile[$offset];
			$offset++;
		}
...
{
	if (!isset($wholefile[$offset]) || !is_int($offset))
		return array(false);
...
function decodeDict($wholefile, $start=0)
{
	...
	while (true)
	{
		...
		if ($left[0] === false || !is_scalar($left[0]))
			return array(false);
		$offset = $left[1];
		if(!isset($wholefile[$offset]) || !is_int($offset)) return array(false);
		if ($wholefile[$offset] == 'd')
